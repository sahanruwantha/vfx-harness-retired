"""Is the run on track, and are the newer capabilities actually being used?
    ...
    python -m pipeline.inspect_run <shot> --layer 3    # one layer, with its timeline
    python -m pipeline.inspect_run <shot> --tools      # tool adoption only
    python -m pipeline.inspect_run <shot> --json
...
`logs/run_layer*.json`, adoption from grepping a console log that no longer existed, and
...
def _trajectory(means: list[float]) -> str:
    ...
    gain = last - first
    drops = sum(1 for a, b in zip(vals, vals[1:]) if b < a - 1e-9)
...
def layers(shot_folder: Path) -> list[dict]:
    out = []
    for p in sorted((shot_folder / "logs").glob("run_layer*.json"),
...
def report(d: dict) -> str:
    ...
    for p in d["layers"]:
        ...
        L.append(f"     {mark} {str(p['layer']):<3} {(p['title'] or '')[:34]:<34} "
    ...
    for e in transcript.read(path):
        ...
        if k == "prompt":
            ...
            out.append(f"{stamp}▣ {k}: "
                       f"{json.dumps({a: b for a, b in e.items() if a not in ('kind', 'dt', 'seq')}, default=str)[:200]}")
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(prog="pipeline.inspect_run", description=__doc__.split("\n")[0])
