...
what `docs/probes/spike_checks.py` drives.
"""
...
# Loaded BY PATH, not as `pipeline.geom`. Blender ships its own Python without the repo's
# dependencies, so importing the package runs `pipeline/__init__.py` and dies on
...
def check_visibility(name: str, frame: int, samples: int = 27) -> dict:
    ...
    # 3×3×3 grid through the bbox, inclusive of corners
    xs = sorted({c.x for c in corners})
    ys = sorted({c.y for c in corners})
    zs = sorted({c.z for c in corners})
    # bound_box is 8 corners; take min/max per axis and lerp
    ...
    for pt in pts:
        ...
        if dist < 1e-8:
            continue
        direction.normalize()
        hit, loc, _n, _i, hit_obj, _m = sc.ray_cast(deps, origin, direction, distance=dist + 1e-4)
        if not hit:
            missed += 1
            continue
