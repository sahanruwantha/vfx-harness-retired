"""Check the things that make a run pointless BEFORE it spends anything.
    ...
    python -m pipeline.preflight            # check and report
    python -m pipeline.preflight --strict   # exit 1 if anything is wrong
...
def empty_success(info: dict, tool_calls: int) -> str | None:
    ...
    return (f"the session reported '{info.get('subtype')}' after "
            ...
            f"says success). Run `python -m pipeline.preflight`.")


def main(argv: list[str] | None = None) -> int:
    import argparse
    ap = argparse.ArgumentParser(prog="pipeline.preflight")
