...
shipped as `bambi evals grounding`, the broken metric it found (3.3c) is fixed in both of its
implementations, and 3.1/3.3 shipped as `bambi evals plan` + `bambi plan --until-clean`;
...
| `distill_queue.jsonl` | drained only by a manual `bambi_vfx.distill` call |
...
`LAYER_REFERENCES_DESIGN.md` states three of those defects in one sentence. `docs/LIGHTING_FINDING.md`
...
| **0.2 Recalibrate the noise floor** | `bambi_vfx.evals variance` crossed over judge model × prompt template × temperature × repetition; report ICC(2,1) and the between/within split | `_JUDGE_SD = 0.603` is a Fable-5 number. The real floor is likely 40–60% higher because the current estimate omits prompt and model variance |
...
Landed as a single `check_scene(kind=…)` tool in `src/bambi_vfx/blender/tools.py:516` with
kinds `visibility · framing · motion · mesh · scale · passes · bbox`. Each is gated on a
**known-bad fixture** via `bambi evals checks`, which builds one deliberately broken scene
...
— **SHIPPED.** `bambi evals grounding`, `src/bambi_vfx/eval/grounding.py`.
...
`src/bambi_vfx/agents/builder.py:954` takes the median of the panel's means, commented
...
spread, set the band wider. `src/bambi_vfx/eval/determinism.py` already does most of this.
