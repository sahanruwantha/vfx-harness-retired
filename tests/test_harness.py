...
def main():
    ...
    with tempfile.TemporaryDirectory() as _workdir:
        ...
        (_workroot / "logs" / "worklist_2.json").write_text(
    ...
    with tempfile.TemporaryDirectory() as _evdir:
        ...
        from bambi_vfx.scene_checks import KIND_DEFINITIONS as _scene_definitions
        ...
        from bambi_vfx.scene_checks import _control_script as _scene_control_script
        ...
        from bambi_vfx.contracts import active_for as _active_contract
    ...
    check("the prose done-check parser is gone", not (Path("src/bambi_vfx/eval/done_checks.py")).exists())
    ...
    check("nothing is recorded when unbound", (_cl.record(_R()), True)[1] and not (_cs / "logs/cost.jsonl").exists())
    ...
    async def sb():
        ...
        return (
            await dec("/home/sahan/Desktop/bambi-vfx/refs/f100_city.jpg"),
    ...
    from bambi_vfx.guardrails import execution_authority_guard
    ...
    from bambi_vfx.build_prompts import axes_own_look, builder_system, ticket_guidance_names
    ...
    quiet = [
        f"{f.name}:{n.lineno}"
        for f in Path("src/bambi_vfx").rglob("*.py")
        if f.name in crit
    ...
    _fm = "---\nname: x\nverified: %s\n---\nbody\n"
    check("recipe claiming verified:true is blocked", _denied("src/bambi_vfx/recipes/x.md", _fm % "true"))
    check("recipe with verified:false is allowed", not _denied("src/bambi_vfx/recipes/x.md", _fm % "false"))
    check(
    ...
    fixed = {
        "src/bambi_vfx/blender/session.py": "artifact sweep failed",
    ...
    check(
        ...
        "renders/zzz_new.png" in rec2["renders"] and "renders/zzz_new.png" not in rec["renders"],
    )
    ...
    check(
        ...
        not in [
            ln
            for ln in Path("src/bambi_vfx/eval/variance.py").read_text(encoding="utf-8").splitlines()
