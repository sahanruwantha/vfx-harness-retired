...
ZERO tests and exited 0 — the real suite is `tests/test_harness.py`, a print-based script
...
def test_deterministic_harness_passes():
    ...
    p = _run(sys.executable, "-m", "tests.test_harness")
    if p.returncode != 0:
        # The harness prints its own failure list; surface it instead of a bare exit code.
        pytest.fail(f"tests.test_harness exited {p.returncode}\n"
                    f"{p.stdout[-4000:]}\n{p.stderr[-2000:]}")
