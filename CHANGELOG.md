...
- A composed layer renders one plate per observation medium, and a contract is measured on
  the plate it was paid on
  ...
  the layer stopped on a false failure. A layer now owes one group per debt plus one per unit
  medium no debt covers; the extra group measures executable contracts and takes no look
  vote, and composition refuses when an image contract is covered by no group at all.
