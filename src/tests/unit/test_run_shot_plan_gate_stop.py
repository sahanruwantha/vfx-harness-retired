...
driver read only the subprocess exit code, so no transaction existed and every later
invocation repeated the same rejection for no progress.

Scope follows ownership: a finding another layer owns belongs to that layer's own
transaction, a plan-wide finding stays a reviewed global amendment, and a layer view
can only be amended when there is selected authority to amend.
"""
...
class _Recorder:
    """Captures what the boundary published, instead of writing a real stop."""
...
def _drive(
    ...
    *,
    layer: str = "2",
    selected: object = _MISSING,
) -> None:
    resolved = _selected() if selected is _MISSING else selected
    ...
    monkeypatch.setattr(
        run_shot, "_gate_selected_authority", lambda _layout, _shot: (resolved, gate)
    )
...
def test_a_clean_gate_lets_the_layer_build(monkeypatch, boundary) -> None:
    _drive(monkeypatch, boundary, _gate())
    assert boundary.scope is None
    assert boundary.stopped is None
    assert boundary.prepared is None
...
def test_findings_this_layer_owns_become_a_dispatchable_layer_amendment(
    ...
    _drive(monkeypatch, boundary, _gate(_owned("2"), _owned("2")))
    assert (boundary.scope, boundary.layer_id) == ("layer_view", "2")
    assert boundary.stopped == "layer-2-plan-gate"


def test_the_compiled_stop_is_prepared_in_the_run_layout(monkeypatch, boundary) -> None:
    """Run 20260903T193713Z-e51397: the stop compiled but was never written.

    ``_stop_after_stage`` consumes only a prepared envelope, so a compiled-but-unwritten
    stop reads as no stop at all and the run fell back to the unclassified boundary
    defect it had just stopped producing.
    """
    _drive(monkeypatch, boundary, _gate(_owned("2")))
    assert boundary.prepared == "envelope:layer_view:2"
    assert boundary.stopped == "layer-2-plan-gate"
...
def test_another_layers_finding_does_not_block_this_layer(monkeypatch, boundary) -> None:
    ...
    _drive(monkeypatch, boundary, _gate(_owned("1", "composition-coverage")))
    assert boundary.scope is None
    assert boundary.stopped is None
...
def test_a_plan_wide_finding_stays_a_reviewed_global_amendment(
    ...
    _drive(monkeypatch, boundary, _gate(_plan_wide()))
...
def test_a_mixed_rejection_escalates_to_the_global_scope(monkeypatch, boundary) -> None:
    ...
    _drive(monkeypatch, boundary, _gate(_owned("2"), _plan_wide()))
...
def test_the_layer_scope_survives_a_mix_of_owners(monkeypatch, boundary) -> None:
    ...
    _drive(monkeypatch, boundary, _gate(_owned("1", "composition-coverage"), _owned("2")))
    assert (boundary.scope, boundary.layer_id) == ("layer_view", "2")
...
def test_without_selected_authority_there_is_no_layer_view_to_amend(
    ...
    _drive(monkeypatch, boundary, _gate(_owned("2")), selected=None)
    assert boundary.scope == "global_plan"
