...
window. What survives in the layer report is real aggregates (rounds, cost, tool
...
Reading it back:
    ...
    vfx inspect <shot> --run <id>                                   # the digest
    jq -r 'select(.kind=="tool_use") | .tool' runs/<id>/logs/transcripts/*/*.jsonl
    jq -r 'select(.kind=="critic") | "\\(.frame) \\(.mean) \\(.verdict)"' runs/<id>/logs/transcripts/*/*.jsonl
"""
...
def _filename_part(value: str) -> str:
    """Make an agent-controlled label safe as one filename component.

    Planner mode labels include paths such as ``VERIFY (auditing plans/global.draft.md)``.
    Treating that text as a filename used to turn the slash into an unintended directory,
    so the most important (verification) transcript silently became incomplete.
    """
...
def bind(shot_folder: str | Path, stage: str, label: str | None = None,
    ...
    `VFXH_NO_TRANSCRIPT=1` turns it off — for the test suite and for anyone who does not
    want a build script's source mirrored into a second file.
    """
    if not Settings.from_environment().transcript_enabled:
        _STATE.clear()
        return None
    rid = run_id or os.environ.get("VFXH_RUN_ID") or "norun"
    stage_part = _filename_part(str(stage))
    layout = run_artifacts.ensure(shot_folder, rid, command=str(stage))
    folder = layout.logs / "transcripts" / stage_part
    name = f"{_filename_part(str(label)) if label else 'session'}.jsonl"
    ...
    _emit("open", stage=stage, label=label, run_id=rid, at=_now(),
          argv_hint=os.environ.get("VFXH_STAGE_ARGV"))
    return path
...
def message(m) -> None:
    ...
    if isinstance(content, list):
        if getattr(m, "error", None):
            _emit("assistant_error", error=m.error)
        if getattr(m, "stop_reason", None) not in (None, "end_turn"):
            _emit("stop", reason=m.stop_reason, model=getattr(m, "model", None))
        for b in content:
...
def find(shot_folder: str | Path, stage: str | None = None,
    ...
    if run_id:
        ...
        if layout:
            hits.extend((layout.logs / "transcripts").glob("**/*.jsonl"))
    else:
        runs = shot / "runs"
        if runs.is_dir():
            hits.extend(runs.glob("*/logs/transcripts/**/*.jsonl"))
    if stage:
        safe = _filename_part(stage)
        hits = [p for p in hits if p.parent.name == safe or p.name.startswith(safe)]
    return sorted(set(hits))


def run_id_for(path: str | Path, shot_folder: str | Path) -> str | None:
    """Return the structured parent run id."""
    p = Path(path).resolve()
    shot = Path(shot_folder).expanduser().resolve()
    try:
        rel = p.relative_to(shot / "runs")
    except ValueError:
        return None
