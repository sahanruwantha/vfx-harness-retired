...
build agent never sees test scaffolding as if it were part of the technique.
    ...
    python -m vfx_harness.knowledge.verify_recipes                 # spike everything, write the ledger
    python -m vfx_harness.knowledge.verify_recipes --name night-city-field
    python -m vfx_harness.knowledge.verify_recipes --audit         # no Blender: claims vs ledger
    python -m vfx_harness.knowledge.verify_recipes --sync          # make frontmatter match the evidence
    python -m vfx_harness.knowledge.verify_recipes --demote        # downgrade only, never upgrade
"""
...
from vfx_harness.knowledge.recipes import RECIPES_DIR, _all
...
def spike(rec: dict, blender: str = "blender", timeout: int = 240) -> dict:
    ...
    base = {"name": rec["name"], "at": datetime.now(UTC).isoformat(timespec="seconds"),
    ...
    if rep["defined"] and not ran:
        return {**base, **evidence, "verdict": "unproven",
                "why": "defines {} but nothing called it; no arguments could be "
                       "synthesised ({}) — add recipes/_spikes/{}.py".format(", ".join(rep["defined"][:3]),
