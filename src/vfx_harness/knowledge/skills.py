...
different fix (index, hook, or helper).
    ...
    python -m vfx_harness.knowledge.skills            # the report
    python -m vfx_harness.knowledge.skills --json     # machine-readable
"""
...
from vfx_harness.infrastructure.config import PROJECT_ROOT
from vfx_harness.knowledge.recipes import search_recipes

SHOTS = PROJECT_ROOT / "shots"


def _unproven() -> set[str]:
    """Recipes with no live spike evidence (see `vfx_harness.knowledge.verify_recipes`).
    ...
    from vfx_harness.knowledge.recipes import _all
    from vfx_harness.knowledge.verify_recipes import audit, load_ledger
...
def report(data: dict) -> str:
    ...
    if blind:
        lines.append(f"  → and these covering recipes have NO spike evidence: "
                     f"{', '.join(blind)}. Run `python -m vfx_harness.knowledge.verify_recipes "
