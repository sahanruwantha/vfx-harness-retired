...
`artifacts/evaluations/baselines/<shot>/`.
...
def _layer_log(shot: Shot, layer_id: str) -> dict:
    """The structured per-layer run report — cost, tokens, turns, hooks.
    ...
    """
    layout = run_artifacts.select(shot.folder)
    p = (layout.reports / "layers" / f"layer-{layer_id}.json" if layout
         else shot.folder / "runs" / "missing" / f"layer-{layer_id}.json")
    if not p.is_file():
        return {"present": False, "why": f"structured layer-{layer_id}.json report not written"}
    try:
...
def freeze(shot: Shot, *, label: str = "", note: str = "",
    ...
    if rdir.is_dir():
        for p in sorted(rdir.iterdir()):
            if p.is_file():
                renders[p.relative_to(shot.folder).as_posix()] = _sha(p)
