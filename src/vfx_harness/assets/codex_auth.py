...
def should_refresh(data: dict) -> bool:
    ...
    if exp:
        return datetime.now(UTC) + REFRESH_WINDOW >= exp
...
def _lock():
    ...
    path = codex_home() / ".vfx-refresh.lock"
...
def refresh(data: dict | None = None, *, force: bool = False) -> dict:
    ...
    with _lock():
        ...
        merged["last_refresh"] = (datetime.now(UTC)
...
def status() -> dict:
    ...
    return {
        ...
        "expires_in_hours": round((exp - datetime.now(UTC)).total_seconds() / 3600, 2)
        if exp else None,
