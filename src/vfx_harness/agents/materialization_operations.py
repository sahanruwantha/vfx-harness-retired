"""Provider-neutral VFX materialization operations and their exact authority checks."""
...
from vfx_harness.agents.materialization_contracts import observation, operation
...
def materialization_operations(**closed):

    write_guard = closed.get("write_guard", _current_selection_guard)
    ...
    async def stage_materialization_unit_tool(args):
        ...
        try:
            ...
            async with materialization_write_lock:
                ...
                staged = await anyio.to_thread.run_sync(
                    lambda: stage_materialization_unit(
                        ...
                        candidate_write_guard=lambda: write_guard(
                            shot_folder,
                            authority.selected,
                        ),
            ...
            return observation(f"unit staging refused: {exc}", is_error=True)
        ...
        return observation(
            ...
            + open_findings,
            data={"unit_id": compiled_unit["id"], "remaining_findings": remaining},
        )
    ...
    async def unstage_materialization_unit_tool(args):
        ...
        try:
            ...
            return observation(f"unit unstaging refused: {exc}", is_error=True)
        return observation(
            ...
            f"{len(payload['scene_contracts'])} contract(s), and "
            f"{len(payload['requirement_bindings'])} requirement binding(s).",
            data={"unit_id": result.unit_id, "removed_contract_ids": list(result.removed_contract_ids),
                  "removed_requirement_ids": list(result.removed_requirement_ids)},
        )
    ...
    async def materialization_status(args):
        ...
        try:
            ...
            return observation(f"materialization status unavailable: {exc}", is_error=True)
        progress = {
            "staged_units": units,
        ...
        return observation(json.dumps(progress), data=progress)
    ...
    async def finalize_materialization(args):
        ...
        try:
            ...
            return observation(str(exc), is_error=True)
        if not findings:
            try:
                ...
                return observation(f"terminal materialization gate failed: {exc}", is_error=True)
            ...
            if not owned.clean:
                ...
                return observation(
                    ...
                    is_error=True,
                    data={"gate_clean": False, "gate": owned.to_dict()},
                )
        ...
        return observation(
            ...
            is_error=True,
            data={"local_findings": list(findings)},
        )
    ...
    async def patch_materialization(args):
        ...
        try:
            for index, row in enumerate(raw_patches):
                ...
                if not pointer:
                    return observation(f"patches[{index}].pointer is required", is_error=True)
                value = json.loads(str((row or {}).get("value")))
                patches.append((pointer, _compiled_patch_value(pointer, value)))
        except (TypeError, json.JSONDecodeError) as exc:
            return observation(f"value must be JSON-encoded: {exc}", is_error=True)
        ...
        try:
            ...
            return observation(str(exc), is_error=True)
        if not findings:
            return observation(f"VALIDATION PASSED for {candidate.name}.", data={"local_findings": []})
        detail = "\n".join(f"- {item}" for item in findings)
        return observation(
