...
class UnitAttemptAuthorityLost(RuntimeError):
...
class AttemptBoundBlenderSession:
    """Proxy every Blender operation through one exact attempt guard.
    ...
    def __init__(self, session: Any, guard: UnitAttemptGuard) -> None:
        self._session = session
        self._attempt_guard = guard
    ...
    def __getattr__(self, name: str) -> Any:
        ...
        if name == "snapshot":
            def snapshot(tag: str) -> dict:
                self._attempt_guard.check("start Blender snapshot staging")
                staged = self._session.stage_snapshot(tag)
                self._attempt_guard.check("finish Blender snapshot staging")
                prepared = self._session.prepare_snapshot_publication(staged)
                try:
                    return self._attempt_guard.publish(
        ...
        if name == "journal":
            def journal(*args, **kwargs) -> dict:
                self._attempt_guard.check("start Blender journal staging")
                staged = self._session.stage_journal(*args, **kwargs)
                if staged[1] is None:
                    self._attempt_guard.check("finish Blender journal read")
                    return staged[0]
                self._attempt_guard.check("finish Blender journal staging")
        ...
        def invoke(*args, **kwargs):
            self._attempt_guard.check(f"start Blender session operation {name}")
            result = attribute(*args, **kwargs)
            self._attempt_guard.check(f"finish Blender session operation {name}")
            return result
