...
from vfx_harness.agents.critic_rubric import critic_prompt as critic_prompt
...
_BUILDER_TMPL = """\
You are the BUILD agent in an automated 3D/VFX vfx_harness. You construct a Blender
...
LIGHTING IN A SCENE THAT HAS A WORLD VOLUME — read this before adding a key light:
  ...
  - bvfx_camera_rig(name, lens, spine=[(frame, dist, alt, pitch_up)], ladder=[(frame,
    roll_deg)], role=..., owner_layer=...) — the two-object camera: an EMPTY owns
    location+pitch, the camera child owns ROLL on its own local Z. `role` is REQUIRED
    and must be a role your unit declares in its mutation scope (not the display
    name): both objects persist and are tagged `role` and `<role>.camera`.
    Returns (rig, cam). NEVER roll a bare camera: its
