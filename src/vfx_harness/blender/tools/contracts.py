...
def register_contracts(
    ...
    @tool(
        ...
        "(infeasible proves that no SINGLE axis-aligned box can; a multi-part subject "
        "whose union projections satisfy the rows may still, measured with "
        "contract_result, and cannot_express_in_scope naming the camera provider is the "
        "path when no construction can). Address subjects with role= (bvfx_role); "
