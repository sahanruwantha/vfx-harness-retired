...
_BANNED: list[tuple[re.Pattern, str]] = [
    ...
    (re.compile(r"file_format\s*=\s*['\"]FFMPEG['\"]"),
     ...
     "the harness's job (bambi_vfx.render_shot), not a layer's. Use 'PNG'."),
        ...
        if re.search(r"^verified:\s*true\s*$", body, re.MULTILINE | re.IGNORECASE):
            ...
            return {"hookSpecificOutput": {
                ...
                "permissionDecisionReason":
                    ...
                    "bambi_vfx.verify_recipes after the snippet has been EXECUTED in a "
        ...
        if "SPIKE_ARGS" in body:
            ...
            return {"hookSpecificOutput": {
                ...
                "permissionDecisionReason":
                    ...
                    "a real shot. Put it in bambi_vfx/recipes/_spikes/<slug>.py instead "
