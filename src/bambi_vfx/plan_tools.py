...
def build_plan_tools(shot_folder: Path, *, blender: str = "blender", lab_dir: Path | None = None):
    ...
    async def spike(args):
        ...
        try:
            ...
            return _text(
                f"spike timed out — simplify the rig or raise timeout (script kept: logs/{lab.name}/{py.name})",
                is_error=True,
            )
        except Exception as e:
        ...
        for e in res["errors"][:3]:
            log(f"· {e[:160]}", 2)
        head = f"spike #{n} · exit {res['rc']} in {res['wall']:.1f}s · kept: logs/{lab.name}/{py.name}" + (
