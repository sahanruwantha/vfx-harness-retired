"""Is the run on track, and are the newer capabilities actually being used?
    ...
    python -m bambi_vfx.inspect_run <shot>              # the digest
    python -m bambi_vfx.inspect_run <shot> --layer 3    # one layer, with its timeline
    python -m bambi_vfx.inspect_run <shot> --tools      # tool adoption only
    python -m bambi_vfx.inspect_run <shot> --json
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(prog="bambi_vfx.inspect_run", description=__doc__.split("\n")[0])
