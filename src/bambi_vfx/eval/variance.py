...
def report(rec: dict) -> str:
    ...
    lines = [
        ...
        "   motion     NO strip attached — a proxy for a `motion` shot's production "
        "critic, which sees one more image",
        "",
    ...
    if n < MIN_N_FOR_BAND:
        ...
        lines.append(
            ...
            f"axes).")
        lines.append(
            "   Caveat that travels with that number: it is ONE render/reference pair. "
            "Ambiguity is a property of the frame, so this is a lower bound for the "
            "pipeline as a whole, not an estimate of it.")
...
def main(argv: list[str]) -> int:
    ...
    ap = argparse.ArgumentParser(prog="bambi_vfx.evals variance")
