...
def _builder_render(folder: Path, c) -> Path | None:
    ...
    if c.frame is not None:
        ...
        hits = [q for q in sorted(rd.glob(f"{c.layer}@f{c.frame}_*.png"))
                if "_motion" not in q.name]
        if hits:
            return hits[0]
...
def _check_done(folder: Path) -> tuple[list[Finding], dict]:
    ...
    if not spec.is_file():
        ...
        return [Finding(
            ...
            "a sentence, and three of those shipped in this plan")], {"checks": 0}
    if spec.is_file():
        corpus = sorted(p for sib in folder.parent.glob("*/renders")
                        if sib.parent.name != folder.name
        ...
        try:
            ...
            return [Finding("done-checks", True, "checks.json",
                            f"unreadable: {str(e)[:80]}")], {}
        for c in checks:
            ref = folder / c.ref
            if not ref.is_file():
...
def _check_coverage(folder: Path) -> tuple[list[Finding], dict]:
    ...
    try:
        layers = json.loads((folder / "layers.json").read_text())
        specs = json.loads((folder / "checks.json").read_text())
    except (OSError, json.JSONDecodeError):
        return [], {}
    if not layers:
        return [], {}
    ...
    for lay in layers:
        ...
        if runnable.get(lid, 0) == 0:
            n = sum(1 for c in specs if str(c.get("layer") or "") == lid)
            out.append(Finding(
                "coverage", True, f"layer {lid}",
    ...
    for mo in rec["moments"]:
        ...
        for r in mo["claims"]:
            if r["verdict"] == "MISMATCH":
                out.append(Finding(
                    "grounded", True, f"{mo['id']} {r['key']}",
                    f"plan claims {r['claimed']:g}, the plate reads {r['actual']:g} "
                    f"({r['rel'] * 100:.0f}% off)",
...
def _check_citations(folder: Path, plan: str) -> tuple[list[Finding], dict]:
    ...
    for m in _CITE.finditer(plan):
        ...
        if lineno:
            ...
            if int(lineno) > n:
                out.append(Finding(
                    "citations", True, f"{rel} line {lineno}",
...
def _check_evidence(folder: Path, plan: str) -> tuple[list[Finding], dict]:
    ...
    for (tid, _title, tag), body in zip(tagged, bodies, strict=False):
        ...
        if "spiked" in low:
            ...
            spikes = [f"spike_{n:0{pad}d}.{ext}"
                      for raw in _SPIKE.findall(body) for n in [int(raw)]
                      for pad in (2, 1) for ext in _SPIKE_EXT]
...
def _check_contracts(folder: Path, *, require_scene_checks: bool = False
    ...
    if missing:
        legacy = " (this shot has gates.json — it predates the layer contract)" \
            if (folder / "gates.json").is_file() else ""
        return [Finding(
            "contracts", True, ", ".join(missing),
...
def report(res: GateResult) -> str:
    ...
    head = (f"── plan gate · {res.shot} · "
            + ("CLEAN" if res.clean else f"{n_b} blocking") + " ──")
    lines = [head]
    s = res.stats
    if s:
        lines.append(
            ...
            + (f"{s['checks']} executable checks "
               f"({s.get('checks_unadversaried', 0)} without a named adversary) · "
               if "checks" in s else
               f"{s.get('done_bound', 0)} done-checks bound (prose fallback) · ")
...
def feedback(res: GateResult) -> str:
    ...
    if not b:
        return ""
    lines = [f"A deterministic gate ran against your plan and found {len(b)} blocking "
    ...
    lines += ["", "Do not remove a target merely to silence the gate unless the finding "
