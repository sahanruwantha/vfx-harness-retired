...
def _pixel_contract_gate(
    shot_dir: Path, layer_id: str, *, frame: int, ref: str, render: str | Path
...
def _compare_image(cand_path: str, ref_path: Path, caption: str, *, feedback_groups=None) -> dict:
    ...
    from ..compare_panels import mark_pair
...
def build_blender_tools(
    ...
    async def run_bpy(args):
        ...
        if shot_dir and layer_id:
            try:
                from ..scene_checks import layer_evidence
            ...
            try:
                gate_pass, gate_rows = await anyio.to_thread.run_sync(
                    lambda: _pixel_contract_gate(
                        shot_dir, str(layer_id), frame=int(args["frame"]), ref=str(ref), render=r["image_path"]
                    )
                )
...
"""
    ...
    async def propose_checks(args):
        ...
        if not after.is_file():
            available = sorted(
                (p for base in (root / ".artifacts", root / "renders") if base.is_dir() for p in base.glob("*.png")),
