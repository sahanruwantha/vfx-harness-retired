...
different fix (index, hook, or helper).
    ...
    python -m bambi_vfx.skills            # the report
    python -m bambi_vfx.skills --json     # machine-readable
"""
...
def _unproven() -> set[str]:
    """Recipes with no live spike evidence (see `bambi_vfx.verify_recipes`).
...
def report(data: dict) -> str:
    ...
    if blind:
        lines.append(f"  → and these covering recipes have NO spike evidence: "
                     f"{', '.join(blind)}. Run `python -m bambi_vfx.verify_recipes "
