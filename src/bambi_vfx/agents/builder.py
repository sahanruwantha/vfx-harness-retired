...
Usage:
    python -m bambi_vfx.agents.builder <shot-folder> --layer <id> [--rounds 2]
"""
...
from ..runlog import bump, reset_counts, snapshot_counts
...
Write bambi_vfx/recipes/<slug>.md with frontmatter (name, tags, blender: "5.2+", when,
...
script, not running one. `verified: true` is set ONLY by bambi_vfx.verify_recipes, and only
...
recorded as evidence in bambi_vfx/recipes/_verified.json. `--audit` fails on any recipe
...
Verification is earned, by running:
    ...
    python -m bambi_vfx.verify_recipes --name <slug> --sync
...
also Write bambi_vfx/recipes/_spikes/<slug>.py supplying them. Read
bambi_vfx/recipes/_spikes/README.md first; it documents the SPIKE_ARGS convention. Keep
...
def _script_options(shot: Shot, *, mode: str, script_rel: str) -> ClaudeAgentOptions:
    ...
    return ClaudeAgentOptions(
        ...
        disallowed_tools=(
            ...
            if finalize
            else ["Write", "Bash", "WebFetch", "WebSearch", "Task", "Agent", "NotebookEdit"]
        ),
        permission_mode="bypassPermissions",
        setting_sources=[],
        max_turns=20,
...
async def _run_script_agent(shot: Shot, *, mode: str, script_rel: str, prompt: str, verbose: bool) -> dict:
    ...
    async for message in query(prompt=prompt, options=_script_options(shot, mode=mode, script_rel=script_rel)):
        if verbose:
            log_message(message)
...
def _critic_schema(axes: list[tuple[str, str]], *, allow_na: bool = True) -> dict:
    ...
    return {
        ...
        "required": ["scores", "issues", "issue_evidence", "focus_requests", "reference_usable", "reference_note"],
        "additionalProperties": False,
    }
...
def _filter_critic_issues(verdict: dict, evidence: list[dict] | None) -> dict:
    ...
    for index, issue in enumerate(original):
        ...
        reason = f"cites passing check(s) {', '.join(cited_pass)}" if cited_pass else "has no failed executable check"
...
# Re-measure before trusting the adjudication economics: python -m bambi_vfx.evals variance <shot>
...
def _needs_critic_panel(verdict: dict) -> bool:
    ...
    """
    return not verdict.get("reference_unusable") and verdict.get("decided_by") != "checks" and _borderline(verdict)
...
def _render_evidence(shot: Shot, layer, m: Milestone, render_rel: str, session: BlenderSession) -> list[dict]:
    if layer is None:
        return []
    evidence = []
    stage = (
    ...
    try:
        from ..scene_checks import functional_evidence
...
def _worklist_evidence(shot_folder: str | Path, layer_id: str) -> list[dict]:
    ...
    """
    path = Path(shot_folder) / "logs" / f"worklist_{layer_id}.json"
    if not path.is_file():
        return []
    try:
...
async def build_unit(
    ...
    *,
    ...
    phase = {"mode": "live", "round": 1, "frame": int(m.frame), "look_actions": _look_actions}
    ...
    if layer is not None and int(layer.id) > 1:
        from ..scene_checks import prior_interface_evidence
    ...
    async with ClaudeSDKClient(options=opts) as builder:
        ...
        for attempt in range(1, MAX_CANON_REPAIRS + 1):
            ...
            if last_info["subtype"] in _TRUNCATED:
                log(f"✗ canonical repair TRUNCATED ({last_info['subtype']}) — stopping here", 1)
                break
    ...
    status = "passed" if ok else ("judge_conflict" if canonical == "judge_conflict" else "failed")
    ...
    if ok or _ERRORS:
        ...
        # `python -m bambi_vfx.distill <shot>` after the run, or set BVFX_DISTILL_INLINE=1.
        ...
        if Settings.from_environment().distill_inline:
            ...
            try:
                ...
                log(
                    ...
                    f"`python -m bambi_vfx.distill {shot.folder}`",
                    1,
                )
            except OSError as e:
    ...
    try:
        ...
        rec_path = write_run(
            shot.folder,
            layer if layer is not None else m,
            ...
            extra={
                ...
                "tools": tool_use_summary(
                    motion_owned=_axes_need_motion(axes),
...
async def build_layer(
    ...
    *,
    ...
    done = "\n".join(ln for ln in excerpt.splitlines() if ln.startswith(("**Scope", "**Judge artifact", "**Done")))
    owned = (
    ...
    return await build_unit(
        ...
        layer.script,
        _prior_layer_paths(shot, layer, force=force),
        session,
        rounds=rounds,
        verbose=verbose,
    ...
    if len(shots_) == 1 and live_best_render and live_best_verdict:
        ...
        if reproduction.get("match"):
            ...
            if blocking:
                ...
                log(
                    f"canonical pixels reproduce accepted live render "
...
async def _run(
    ...
    if any("CHANGED since the plan" in s for s in stale) and not force:
        log("   re-plan with `python -m bambi_vfx.agents.planner <folder>`, or --force")
        raise SystemExit(8)
    ...
    if unanswered and not force:
        ...
        log(f'   answer with: python -m bambi_vfx.escalate {folder} --answer <id> "..."')
        raise SystemExit(5)
    if unanswered:
    ...
    try:
        ...
        log(f"build agent: shot '{shot.id}' LAYER {g.id} — {g.title} (judges: {judged}) → {g.script}, model {MODEL}")
...
def main() -> None:
    ...
    # traceback on top would bury both under a stack nobody needs.
        ...
        raise SystemExit(3) from None  # distinct from a crash (1) so drivers can tell them apart
    except ChainBroken as e:
        log(f"CHAIN BROKEN — {e}")
        raise SystemExit(4) from None  # the chain needs repair; building on is pointless
    except UnpassedPrior as e:
        log(f"UNACCEPTED PRIOR — {e}")
        raise SystemExit(6) from None  # a lower layer must pass first
