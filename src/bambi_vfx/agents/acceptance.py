...
run. So acceptance is judged exactly once, here, on the full rubric.
    ...
    python -m bambi_vfx.agents.acceptance <shot-folder> [--moment M2]
...
from ..ledger import Ledger, load_layers, load_milestones
from ..log import log
...
def repair_plan(shot: Shot, results: dict) -> list[dict]:
    ...
    """
    from .agents.builder import PASS_MIN
...
def main() -> None:
    ...
    try:
        ...
        raise SystemExit(7) from None   # the log line above is the message; see build_agent
