...
whose only consumer is a FUTURE layer. Nothing downstream in the current run reads it.
    ...
    python -m bambi_vfx.distill <shot-folder> [--keep]
...
from .agents.builder import distill_recipe
from .brief import load_shot
from .config import load_environment
from .ledger import Milestone, load_layers
from .log import log

QUEUE = "logs/distill_queue.jsonl"
...
async def _run(folder: str, keep: bool) -> None:
    ...
    if not reqs:
        log(f"nothing queued in {shot.folder / QUEUE}")
        return
